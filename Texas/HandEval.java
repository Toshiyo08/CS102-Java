...
public class HandEval extends Card { 
    ...
    protected String evaluateHand(){
        ...
        for(int i=0; i<availableCards.length;i++){
            rankCounter[ availableCards[i].getNumber() ]++;
        ...
        if ((rankCounter[9] >= 1 &&       /* 10 */
            ...
            royalSearch:
                for (int i=0;i<3;i++){
                    ...
                    if (availableCards[i].getNumber() == 0){
                        ...
                        for (int j=1;j<4-i;j++){
                            if ((availableCards[i+j].getNumber() == 9 && 
                                    availableCards[i+j+1].getNumber() == 10 &&
                                    availableCards[i+j+2].getNumber() == 11 &&
                                    availableCards[i+j+3].getNumber() == 12) 
                                    &&
    ...
    private String evaluateStraightFlush(short[] rankCounter, short[] suitCounter){
        ...
        if (suitCounter[0] > 4 || suitCounter[1] > 4 ||
            ...
            for (int i=availableCards.length-1;i>3;i--){
                if ((availableCards[i].getNumber()-ONE == availableCards[i-ONE].getNumber() && 
                        availableCards[i].getNumber()-TWO == availableCards[i-TWO].getNumber() &&
                        availableCards[i].getNumber()-THREE == availableCards[i-THREE].getNumber() &&
                        availableCards[i].getNumber()-FOUR == availableCards[i-FOUR].getNumber()) 
                        &&
                        ...
                        availableCards[i].getSuit() == availableCards[i-FOUR].getSuit())){
                            // Found royal flush, break and return.
                            result = "Straight Flush!! " + Card.rankAsString(availableCards[i].getNumber()) + " high of " + Card.suitAsString(availableCards[i].getSuit());
                            break;
                }
            }
        }
        ...
        if (suitCounter[0] > 4 || suitCounter[1] > 4 ||
            ...
            for (int i=availableCards.length-1;i>3;i--){
                if (availableCards[i].getSuit() == availableCards[i-ONE].getSuit() &&
                        ...
                        availableCards[i].getSuit() == availableCards[i-FOUR].getSuit()){
                            // Found royal flush, break and return.
                            result = "Flush!! " + Card.rankAsString(availableCards[i].getNumber()) + " high of " + Card.suitAsString(availableCards[i].getSuit());
                            break;
                }
            }           
        }
